...
public class ApiinfoController {
    ...
    @ApiOperation("接口列表")
    @SaCheckPermission("apiinfo_view")
    ...
    @SysLog("接口修改")
    @ApiOperation("接口修改")
    @SaCheckPermission("apiinfo_edit")
    @PutMapping("/update")
    ...
    @SysLog("接口删除")
    @ApiOperation("接口删除")
    @SaCheckPermission("apiinfo_del")
    @DeleteMapping("/remove/{id}")
    public R remove(@PathVariable("id") Integer[] id) throws Exception {
        return R.ok(apiinfoService.removeByIds(Arrays.asList(id)));
    }

    @SaCheckPermission("apiinfo_export")
    @GetMapping("/export")
    public void export(Apiinfo apiinfo, ModelMap map, HttpServletRequest request, HttpServletResponse response) {
        List<Apiinfo> list = apiinfoService.list(getQueryWrapper(apiinfo));
        ExcelUtil.exportExcel("接口信息", list, Apiinfo.class, map, request, response);
    }
}
