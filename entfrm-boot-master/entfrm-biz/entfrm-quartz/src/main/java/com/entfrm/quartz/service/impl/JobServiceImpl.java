...
import com.entfrm.quartz.mapper.JobMapper;
...
public class JobServiceImpl extends ServiceImpl<JobMapper, Job> implements JobService {
    ...
    public void init() throws SchedulerException {
        scheduler.clear();
        List<Job> jobList = baseMapper.selectList(new QueryWrapper<>());
        for (Job job : jobList) {
    ...
    public void run(Job job) throws SchedulerException {
        Integer jobId = job.getId();
        String jobGroup = job.getJobGroup();
        Job properties = baseMapper.selectById(jobId);
        // 参数
    ...
    public int updateJob(Job job) throws SchedulerException {
        Job properties = baseMapper.selectById(job.getId());
        int rows = baseMapper.updateById(job);
        if (rows > 0) {
